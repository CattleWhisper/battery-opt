...
| `sensor.battery_opt_best_periods` | Start of the next not-yet-ended best appliance period (timestamp device class) across today and tomorrow, from the delivered-price vectors at the shared defaults (maximal cheap stretches at or below min + 30% of the day's range, ≥ 30 min, top 3); attributes carry both days' lists in time order (`periods` / `tomorrow_periods`, `{start, end, avg_price_eur_kwh}`), the mirrored expensive tier (`expensive_periods` / `tomorrow_expensive_periods` — maximal runs at or above max − 50% of the range, deliberately wider than cheap: the tiers steer the HUMAN toward cheap energy and are read by no planner; for the traffic-light day strip) plus each day's cheap cutoff and average price — the dashboard face of the `get_best_periods` service, one detection, two faces |
...
default 30 min), `threshold` (percent of the range, default 30),
